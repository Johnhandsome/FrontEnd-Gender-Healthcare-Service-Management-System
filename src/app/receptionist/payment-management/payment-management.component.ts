...
export class PaymentManagementComponent implements OnInit {
  ...
  generatePaymentRecords(patients: Patient[]): PaymentRecord[] {
    ...
    return patients.map((patient, index) => ({
      ...
      payment_date: Math.random() > 0.5 ? new Date().toISOString() : undefined,
  ...
  generateRandomServices(): ServiceUsage[] {
    ...
    for (let i = 0; i < serviceCount; i++) {
      randomServices.push({
        service_id: `SRV-${i}`,
        service_name: ['Consultation', 'Blood Test', 'X-Ray', 'Ultrasound'][Math.floor(Math.random() * 4)],
  ...
  getRandomStatus(): 'pending' | 'paid' | 'partial' | 'overdue' {
    const statuses: ('pending' | 'paid' | 'partial' | 'overdue')[] = ['pending', 'paid', 'partial', 'overdue'];
  ...
  async calculateStats() {
    this.stats = {
      ...
      pendingPayments: this.paymentRecords
        .filter(r => r.payment_status === 'pending').length,
      paidToday: this.paymentRecords
        .filter(r => r.payment_status === 'paid' && this.isToday(r.payment_date)).length,
      overduePayments: this.paymentRecords
        .filter(r => r.payment_status === 'overdue').length
    };
  }
  ...
  applyFilters() {
    this.filteredRecords = this.paymentRecords.filter(record => {
      const matchesSearch = !this.searchQuery ||
        record.patient_name.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
        record.patient_email.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
        record.payment_id.toLowerCase().includes(this.searchQuery.toLowerCase());

      const matchesStatus = !this.selectedStatus || record.payment_status === this.selectedStatus;

      const matchesDateRange = !this.selectedDateRange || this.isInDateRange(record.created_at, this.selectedDateRange);
  ...
  getTotalAmount(): number {
    return this.newPayment.services.reduce((total, service) =>
      total + (service.service_price * service.quantity), 0);
  }
  ...
  async updatePaymentStatus(record: PaymentRecord, event: any) {
    ...
    try {
      ...
      // Update in the array
      const index = this.paymentRecords.findIndex(r => r.payment_id === record.payment_id);
      if (index !== -1) {
  ...
  async createPayment() {
    ...
    try {
      ...
      const patient = this.patients.find(p => p.id === this.newPayment.patient_id);
      if (!patient) {

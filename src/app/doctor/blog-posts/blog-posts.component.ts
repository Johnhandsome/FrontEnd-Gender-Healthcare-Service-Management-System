...
export class BlogPostsComponent implements OnInit {
  ...
  async saveBlogPost() {
    ...
    try {
      ...
      // Ensure doctor is authenticated with Supabase before saving
      await this.ensureSupabaseAuthentication();

      const formData = this.blogForm.value;

      // Clean the blog content before saving to remove unnecessary inline styles
      const cleanedContent = this.cleanHtmlContent(formData.blog_content || '');

      const blogData: CreateBlogPostRequest = {
      ...
      console.log('🧹 Original content length:', formData.blog_content?.length || 0);
      ...
      if (this.isEditMode && this.currentPost) {
        ...
        await this.supabaseService.createBlogPost(this.doctorId, blogData);
        console.log('✅ Created post in Supabase');
      }
  ...
  // Clean HTML content by removing unnecessary inline styles
